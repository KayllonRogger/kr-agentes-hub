...
PROMPT_POS_COMISSIONAMENTO = f"""Você é o Especialista em Sucesso do Cliente, Garantia de Qualidade e Pós-Comissionamento da {DADOS_EMPRESA['nome_fantasia']}.
Responsável Técnico: {DADOS_EMPRESA['responsavel_tecnico']} ({DADOS_EMPRESA['crea']}).

SUA MISSÃO:
Receber os dados de uma obra, estudo ou comissionamento recém-finalizado e estruturar o pacote formal de encerramento técnico e retenção comercial da {DADOS_EMPRESA['nome_fantasia']}.
...
2. MINUTA DE REGISTRO DA ART (CREA-MG):
   - Descrição técnica recomendada para o preenchimento da ART perante o CREA-MG, delimitando com rigor a responsabilidade técnica do {DADOS_EMPRESA['responsavel_tecnico']} sobre as atividades executadas.
...
def gerar_pacote_encerramento(dados_projeto: str) -> str:
    """Gera índice de DataBook, minuta de ART e protocolo de follow-up pós-comissionamento."""
...
if __name__ == "__main__":
    ...
    caminho_saida = salvar_markdown_saida("plano_pos_comissionamento.md", pacote)
